  ...
  methods: {
    keyDown(event) {
      ...
      if (key === this.keys.RIGHT ||
          ...
          key === this.keys.ENTER ||
          key === this.keys.DOWN ||
          key === this.keys.PAGE_DOWN) {
        this.nextPage(this.animcursorCheck());
        this.animcursor += 1;
      }
      if (key === this.keys.LEFT ||
          key === this.keys.BACKSPACE ||
          key === this.keys.PAGE_UP) {
        this.animcursor -= 1;
        this.nextPage(this.animcursorCheck());
      }
    },
    animcursorCheck() {
      ...
      if (this.animcursor > this.animations.max) {
        this.animcursor = 1;
      } else if (this.animcursor < 1) {
        this.animcursor = this.animations.max;
      }
    ...
    resetPage($outpage, $inpage) {
      ...
      const oi = $inpage.data('originalClassList');
      $inpage.attr('class', `${oi} pt-page-current`);
    },
  },
};
